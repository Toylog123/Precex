...
def aggregate(results):
    ...
    for r in results:
        ...
        agg["repair_pass"] += 1 if r.get("repair_pass") else 0
        agg["repair_pass_bmc"] += 1 if r.get("repair_pass_bmc", r.get("repair_pass", False)) else 0
...
def mcnemar(base_map, other_map, keys_all):
    ...
    b_only = o_only = 0
    for k in keys_all:
        b = bool(base_map.get(k, {}).get("repair_pass_bmc", base_map.get(k, {}).get("repair_pass", False)))
        o = bool(other_map.get(k, {}).get("repair_pass_bmc", other_map.get(k, {}).get("repair_pass", False)))
...
def main(argv=None):
    ...
    print("样本/调用数: base=%d other=%d 对齐=%d" % (len(b_map), len(o_map), len(overlap)))

    ba = aggregate(base); oa = aggregate(other)
    print("\n-- 总体指标 --")
    ...
    # 按 setting
    print("\n-- 按 setting --")
    for st in sorted({k[1] for k in keys_all}):
        b_s = [r for k, r in b_map.items() if k[1] == st]
        o_s = [r for k, r in o_map.items() if k[1] == st]
    ...
    # McNemar（配对修复率差异显著性）
    mn = mcnemar(b_map, o_map, keys_all)
    print("\n-- McNemar（修复率 BMC，配对）--")
    print("  不一致对: %d  (%s 仅成功: %d, %s 仅成功: %d)" % (
