...
\caption{各设置的定位 Top-1 精度（柱）与 LLM 成本（折线）。四设置均为干净口径（DeepSeek v4-flash）；A vs C 显著（McNemar p=0.0004，Holm p=0.0024）；其余五对差异均不显著（配对 McNemar，Holm 校正后 $p\ge 0.118$）。}
\label{fig:loc_cost}
\end{figure}
...
\textbf{发现 2：}在干净口径（DeepSeek v4-flash，无泄漏）下，定位精度排序为原始日志（A）64.7\%（Wilson 95\% CI 55.1--73.3）$>$ 因果图（D）56.9\%（47.2--66.1）$>$ 结构化证据（B）55.9\%（46.2--65.1）$>$ 反例语义化（C，DS 摘要版）43.2\%（95/102，CI 33.7--53.2）；成本上 D 最低（0.37 美元/102 次），B 次之（0.40 美元），A 最高（0.60 美元），呈现与旧泄漏版相反的精度/成本权衡。

\textbf{统计显著性（干净口径）：}对逐样本定位结果按（样本，seed）配对做 McNemar 精确检验，报告全部六对对比并经 Holm 校正。
A 对 C 显著（\$p=0.0004\$，率差 \$+21.5\$~pp，95\%~CI \$[+7.9,+35.2]\$，Holm 校正后 \$p_{\mathrm{holm}}=0.0024\$）——
原始日志（A）的定位精度显著高于 DS 摘要版反例语义化（C）。
经 Holm 校正后其余五对均不显著：
A 对 B \$p=0.0495\$（\$+8.8\$~pp，\$[-4.6,+22.2]\$，\$p_{\mathrm{holm}}=0.148\$）、
A 对 D \$p=0.0593\$（\$+7.8\$~pp，\$[-5.5,+21.2]\$，\$p_{\mathrm{holm}}=0.118\$）、
B 对 C \$p=0.0411\$（\$+12.7\$~pp，\$[-1.1,+26.6]\$，\$p_{\mathrm{holm}}=0.148\$）、
C 对 D \$p=0.0343\$（\$-13.7\$~pp，\$[-27.5,+0.1]\$，\$p_{\mathrm{holm}}=0.118\$）、
B 对 D \$p=0.8185\$（\$-1.0\$~pp，\$[-14.6,+12.6]\$，\$p_{\mathrm{holm}}=0.819\$）。
旧泄漏版报告的 B 对 A \$p=0.0035\$ 显著结论已被推翻。
seed 独立性方面，推理使用 temperature=0.2 且 prompt 携带 seed 标识（\S\ref{sec:setup}），
聚合定位率跨 seed 波动为：A 0.0~pp（三 seed 均为 64.7\%，但逐样本存在 5/34 处跨 seed 翻转）、
D 2.9~pp、C 2.9~pp、B 11.8~pp（50.0\%--61.8\%），
说明三-seed 配对提供了真实而非名义的抽样方差，其中 B 设置的噪声最大。
全部对比均报告，包括不显著项。
注意：C 设置当前仅 95/102 条（7 条因 DeepSeek v4-flash 上下文限制缺失），
其 Wilson 95\% CI 为 33.7--53.2\%；统计检验基于 95 条配对样本，正式结论以此口径为准。
