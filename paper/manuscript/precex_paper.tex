...
在覆盖 6 个模块、7 类错误的 34 样本 L3 基准上（DeepSeek v4-flash，无泄漏四设置 408 次），本文以受控实验回答两个问题：反例证据表征如何影响 RTL 缺陷定位与修复，以及自动修复评测的可靠性如何被数据卫生与判据卫生决定。统一 BMC 判据下，四种证据表征的 BMC 回归通过率均达 100\%（操作定义：候选补丁在有限深度 BMC 下未产生新反例）；定位精度为原始日志（A）64.7\%、因果图（D）56.9\%、结构化证据（B）55.9\% 与反例语义化（C，DS 摘要版）46.1\%（102/102），A 与 C 差异显著（配对 McNemar p=0.0004，Holm 校正后 p=0.0024），其余五对均不显著——证据表征的定位优势被模型、数据卫生与反例时长共同调节，而非固有属性；深时序子集上排序反转（s39--s42：C 75.0\% > A 66.7\% > D 58.3\% > B 41.7\%，n=12/设置；全量 n=15/设置方向一致）。成本最低的是 D（0.37 美元/102 次），B 次之（0.40 美元），A 最高（0.60 美元）。变异分析杀死 88.5\% 的强变异体与 81.8\% 的常量变异体；独立 T2 审计 306/306 全部通过，接口变更与断言篡改均为零。本文进一步报告一项数据卫生教训：初版实验存在三处 ground-truth 泄漏（注入行号与描述进入 prompt/证据），导致“B 显著最优”的伪结论；剥离泄漏后 68 处定位结果翻转、结论反转。判据翻案审计表明，prove/k-归纳在有门控时序断言的模块上会误判 78 个正确修复，这促使我们采用 golden-first 的 BMC 判据。L2 门禁对照（24 样本，72 次）显示 BMC 回归通过率为 91.7\%（系统把绝大多数单周期错误当作 L3 修复到三通过），弱 tb 门禁不构成能力分界。
\end{abstract}
...
\item \textbf{干净口径的证据表征消融（探索性结论）}：统一 BMC 判据下四种设置 BMC 回归通过率均达 100\%；定位精度 A 64.7\% > D 56.9\% > B 55.9\% > C 46.1\%（102/102），A 与 C 差异显著（McNemar p=0.0004，Holm 校正 p=0.0024），其余五对均不显著——证据表征的定位优势被模型与数据卫生强烈调节，而非固有属性；成本 D 最低（\$0.37），B 次之（\$0.40），A 最高（\$0.60）。
...
C 反例语义化 \& 102 \& 46.1\%（102/102） \& 100\% \& 1.44M \& 0.28 \\\\
...
\textbf{发现 2：}在干净口径（DeepSeek v4-flash，无泄漏）下，定位精度排序为原始日志（A）64.7\%（Wilson 95\% CI 55.1--73.3）$>$ 因果图（D）56.9\%（47.2--66.1）$>$ 结构化证据（B）55.9\%（46.2--65.1）$>$ 反例语义化（C，DS 摘要版）46.1\%（102/102，CI 37.1--56.2）；成本上 D 最低（0.37 美元/102 次），B 次之（0.40 美元），A 最高（0.60 美元），呈现与旧泄漏版相反的精度/成本权衡。


\textbf{缺失样本分析：}C 设置缺失 7 个样本：s33（axi\_lite\_slave，状态跳转）全部 3 个 seeds、s34（axi\_lite\_slave，边界回绕）seed 1、s37（uart\_rx，边界回绕）全部 3 个 seeds。缺失非随机：全部为协议模块（axi/uart），其 buggy.v 更大（211--307 行），semantics.json 中的 fault\_cone 信号更多（14--31 个）。这些样本的语义化证据段在 DeepSeek v4-flash API 上持续出现墙钟超时（wall-clock timeout，3 次重试均失败；非上下文容量限制——实测单次调用 input tokens 仅 7--14K，远低于 DeepSeek v4-flash 的 1M 上下文窗口，超时属 API 端响应不稳定所致）。对 102 个成功样本按模块分层比较：axi 模块（s16--s28，缺 s33/s34）A/C 定位差为 52.9\% vs 40.0\%（\$\Delta=-12.9\$~pp），uart\_rx 模块（s29--s31，缺 s37）为 50.0\% vs 33.3\%（\$\Delta=-16.7\$~pp），较小模块（fifo/fsm/counter/uart\_tx，21 样本无缺失）为 76.2\% vs 50.8\%（\$\Delta=-25.4\$~pp）。缺失样本集中于协议模块，其定位难度本就高于较小模块，实际 C 的 46.1\% 可能是\textbf{高估}（若缺失的 7 次全部失败，全量 102 定位将更低）。论文如实报告 102/102 为当前最佳可用数据。
...
\begin{itemize}
\item 统一 BMC 判据（golden-first）下，四种证据表征的 BMC 回归通过率均达 100\%（有限深度口径）；在干净数据上定位精度 A 64.7\% > D 56.9\% > B 55.9\% > C 46.1\%（102/102）。A 与 C 差异显著（McNemar p=0.0004，Holm 校正后 p=0.0024），其余五对配对检验均不显著——证据表征决定定位的精度与成本，但效应被模型、数据卫生与反例时长共同调节：深时序子集（s39--s42）上排序反转为 C 75.0\% > A 66.7\% > D 58.3\% > B 41.7\%（第 V-I 节）。
