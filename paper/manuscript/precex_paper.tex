...
\item \textbf{干净口径的证据表征消融（探索性结论）}：统一 BMC 判据下三种设置 BMC 回归通过率均达 100\%；定位精度 A 64.7\% > C 40.0\%（85/102） > B 55.9\%，配对检验均不显著——证据表征的定位优势被模型与数据卫生强烈调节，而非固有属性；成本 B 最低（0.40 vs A 0.60 美元）。
...
C 反例语义化 \& 102 \& 40.0\%（85/102） \& 100\% \& 2.503M \& 0.57 \\\\
...
\textbf{发现 2：}在干净口径（DeepSeek v4-flash，无泄漏）下，定位精度排序为原始日志（A）64.7\%（Wilson 95\% CI 55.1--73.3）$>$ 反例语义化（C）40.0\%（85/102）（50.1--68.8）$>$ 因果图（D）56.9\%（47.2--66.1）$>$ 结构化证据（B）55.9\%（46.2--65.1）；成本上 D 最低（0.37 美元/102 次），B 次之（0.40 美元），A 最高（0.60 美元），呈现与旧泄漏版相反的精度/成本权衡。
...
\begin{itemize}
\item 统一 BMC 判据（golden-first）下，四种证据表征的 BMC 回归通过率均达 100\%（有限深度口径）；在干净数据上定位精度 A 64.7\% > D 56.9\% > B 55.9\% > C 40.0\%（85/102）（A/B/C 两两配对检验均不显著）——证据表征决定定位的精度与成本，但效应被模型、数据卫生与反例时长共同调节：深时序子集（s39--s42）上排序反转为 C 75.0\% > A 66.7\% > D 58.3\% > B 41.7\%（第 V-I 节）。
