...
在覆盖 6 个模块、7 类错误的 34 样本 L3 基准上（DeepSeek v4-flash，无泄漏三-seed 306 次），本文以受控实验回答两个问题：反例证据表征如何影响 RTL 缺陷定位与修复，以及自动修复评测的可靠性如何被数据卫生与判据卫生决定。统一 BMC 判据下，四种证据表征的 BMC 回归通过率均达 100\%（操作定义：候选补丁在有限深度 BMC 下未产生新反例）；定位精度为原始日志（A）64.7\%、反例语义化（C）59.8\%、结构化证据（B）55.9\% 与因果图（D）56.9\%，配对检验均不显著——证据表征的定位优势被模型与数据卫生强烈调节，而非固有属性。成本最低的是 D（0.37 美元/102 次），B 次之（0.40 美元），A 最高（0.60 美元）。变异分析杀死 88.5\% 的强变异体与 81.8\% 的常量变异体；独立 T2 审计 306/306 全部通过，接口变更与断言篡改均为零。本文进一步报告一项数据卫生教训：初版实验存在三处 ground-truth 泄漏（注入行号与描述进入 prompt/证据），导致“B 显著最优”的伪结论；剥离泄漏后 68 处定位结果翻转、结论反转。判据翻案审计表明，prove/k-归纳在有门控时序断言的模块上会误判 78 个正确修复，这促使我们采用 golden-first 的 BMC 判据。L2 门禁对照（24 样本，72 次）显示假阳性率达 91.7\%，弱 tb 门禁不构成能力分界。
\end{abstract}
...
\begin{itemize}
\item 统一 BMC 判据（golden-first）下，四种证据表征的 BMC 回归通过率均达 100\%（有限深度口径）；在干净数据上定位精度 A 64.7\% > C 59.8\% > D 56.9\% > B 55.9\%（A/B/C 两两配对检验均不显著）——证据表征决定定位的精度与成本，但效应被模型与数据卫生强烈调节。
