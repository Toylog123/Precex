...
\textbf{（未在主实验启用）}系统代码已实现反馈循环机制：每次失败尝试的 diff 与失败原因被写入历史记录并注入下一轮 prompt，要求 LLM 分析上次失败后再给出不同方案。该机制面向后续扩展评估；干净口径主实验（A/B/C 299 次）与 L2 对照（72 次）均运行于固定 prompt 协议（无历史注入），报告的定位与修复数字反映该协议下的行为。
...
\textbf{缺失样本分析：}C 设置缺失 7 个样本：s33（axi\_lite\_slave，状态跳转）全部 3 个 seeds、s34（axi\_lite\_slave，边界回绕）seed 1、s37（uart\_rx，边界回绕）全部 3 个 seeds。缺失非随机：全部为协议模块（axi/uart），其 buggy.v 更大（211--307 行），semantics.json 中的 fault\_cone 信号更多（14--31 个）。这些样本的 C 证据段总 token 超出 DeepSeek v4-flash 的 128K 上下文限制，导致 API 持续超时（3 次重试均失败）。对 95 个成功样本按模块分层比较：axi 模块（s16--s28，缺 s33/s34）A/C 定位差为 52.9\% vs 40.0\%（\$\Delta=-12.9\$~pp），uart\_rx 模块（s29--s31，缺 s37）为 50.0\% vs 33.3\%（\$\Delta=-16.7\$~pp），较小模块（fifo/fsm/counter/uart\_tx，21 样本无缺失）为 76.2\% vs 50.8\%（\$\Delta=-25.4\$~pp）。缺失样本集中于协议模块，其定位难度本就高于较小模块，实际 C 的 43.2\% 可能是\textbf{高估}（若缺失的 7 次全部失败，全量 102 定位将更低）。论文如实报告 95/102 为当前最佳可用数据。
...
\textbf{外部效度：}基准含 34 个教学级模块样本（fifo/uart/axi-lite/fsm/counter），不宣称工业协议覆盖；样本为确定性错误注入生成而非真实缺陷分布；握手与状态迁移仍是最难定位类别（干净口径 47.2\% 与 30.6\%），uart\_rx 的波特时序常量变异杀死率弱（常量 40.9\%），是结构断言对时序常量漂移不敏感的已记录局限。常见模块可能出现在 LLM 训练语料中，本文以构造日期记录、逐样本来源与错误类别多样性缓解，但无法完全排除残余记忆。干净主实验 299 次运行均使用 DeepSeek v4-flash（A 102/B 102/C 95）；初版 MiniMax M3 实验因数据泄漏整体作废，仅作为卫生教训的对照引用，其干净复跑待后续（表 5 MM 列待补），故本文的证据表征结论限定于所测模型与干净协议，绝对值可能随模型版本变化。另，L2 门禁对照的干净结果见 \S\ref{sec:l2}。
