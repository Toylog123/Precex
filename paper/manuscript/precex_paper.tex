...
\subsection{非 LLM 基线}\subsection{非 LLM 基线}
...
本文提出 PreCex，一个开源、反例驱动的综合前 RTL 缺陷定位与修复流水线，并系统研究了此前反例理解类系统未受控的证据表征维度。在含 408 次 LLM 修复运行的 34 样本跨周期（L3）基准上，主要结论如下：
...
\textbf{外部效度：}基准含 34 个教学级模块样本，不宣称工业协议覆盖；握手与状态迁移仍是最难定位类别（27.8\% 与 36.1\%），uart\_rx 的波特时序常量变异杀死率弱（常量 40.9\%），是结构断言对时序常量漂移不敏感的已记录局限。常见模块可能出现在 LLM 训练语料中，本文以构造日期记录、逐样本来源与错误类别多样性缓解，但无法完全排除残余记忆。408 次运行均使用 MiniMax M3，绝对值可能随模型版本变化，本文主张的是证据表征的相对效应。
