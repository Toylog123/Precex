...
4. **基础设施修正**：diff 应用器模糊空白匹配修复（39 条失败离线重放，10 条转 PASS）；4 条网络超时补跑，s30/A2 PASS，3 条（s16/A0、s16/C0、s17/B2）多次长超时补跑仍未完成（LLM 慢响应 + axi/uart prove 验证双重瓶颈，超单轮窗口），如实标注 net_fail（不影响相对结论）。
...
| uart_tx | 33% | 67% | 25% | **75%** | 9 |
| uart_rx | 42% | 50% | **50%** | 50% | 12 |
| axi_lite_slave | **76%** | 0% | 50% | 0% | 17 |

要点：
1. **修复率边界由模块复杂度决定，而非证据表示**：counter/fsm/fifo 简单模块 B/C 均 100%；
   axi_lite_slave 全部 0%（B/C 相同，握手/复位/状态跳转注入跨周期语义复杂）；
   uart 居中（tx 67-75%、rx 50%）。
