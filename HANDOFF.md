...
已完成：M0.5 预研（Gate-0/1）→ M1 数据集 34 样本定案（Gate-2 通过）→ 主实验 408 次（BMC 100%）→ 跨模型重跑 306 次（DeepSeek 三-seed）→ L2 假阳性率 72 次（91.7%）→ 论文加固 + 终审（9 页 PDF，账本 1519 次 $18.48）；多来源 LLM 可解释性评分（DeepSeek v4-flash + MiniMax M3，40 次，ICC 已写入论文）、BMC 深度敏感性抽查（axi 16→24、uart_rx 24→32）、中文期刊适配调研、投稿包 SHA256SUMS 校验和均已完成；全部已推送（main 最新 6554c71）
