...
- **进行中（M1 数据集，W4–7）**：
  ...
  - **论文加固与终审（2026-08-05）**：新增跨模型/L2 两小节，全文一致性修正（B 优势限定 MiniMax），账本更新 1519 次 $18.48，8 页 PDF 编译验证
...
已完成：M0.5 预研（Gate-0/1）→ M1 数据集 34 样本定案（Gate-2 通过）→ 主实验 408 次（BMC 100%）→ 跨模型重跑 306 次（DeepSeek 三-seed）→ L2 假阳性率 72 次（91.7%）→ 论文加固 + 终审（9 页 PDF，账本 1519 次 $18.48）；多来源 LLM 可解释性评分（DeepSeek v4-flash + MiniMax M3，40 次，ICC 已写入论文）、BMC 深度敏感性抽查（axi 16→24、uart_rx 24→32）、中文期刊适配调研、投稿包 SHA256SUMS 校验和均已完成；全部已推送（main 最新 f6ae54e）

待办（仅剩用户决策项）：
1. **作者单位**：论文 L38 占位符，需填真实单位（软件学报提交后署名冻结）
2. **目标期刊**：首选《软件学报》（CCF T1，需 Word 排版 + 补 3-5 条中文文献），备选《电子学报》（更贴 RTL/EDA）；见 docs/期刊适配调研.md
3. **人工可解释性评分者**：2-3 名真人；若无则保留探索性表述投稿（多 LLM 交叉评分已作为替代）

用户提供上述三项后的执行链：填作者信息 → 期刊模板适配 + 中文文献 → 投稿包冻结重建 → 2026-10 投稿
