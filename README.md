...
- **主创新**：证据表征受控消融（A 原始日志 / B 结构化证据 / C 反例语义化 / D FVDebug 式因果图），B 在 MiniMax 上定位最优（61.8%）且优势模型依赖；
- **差异化三支柱**：① 开源全链路可复现（iverilog/verilator/yosys/sby，无商业依赖）；② 四设置消融方法论（A/B/C/D）+ 跨模型重跑（MiniMax/DeepSeek 三-seed 306 对）；③ 跨周期专项数据集（34 样本）+ 验证充分性闭环（mutation/非空洞/T2 审计/假阳性率）；
...
| --- | --- | --- |
| A | 原始日志/反例原文 | 基线 |
| B | 结构化 JSON | **主设置（MiniMax 上定位最优 61.8%；DeepSeek 上被 C 反超，优势模型依赖）** |
...
- **2026-08-05**：M0.5/M1/M2 全完成——34 样本定案（Gate-2）、主实验 408 次（BMC 100%）、跨模型 306 对、L2 91.7%、论文 8 页 0 overfull；M3 投稿准备进行中（投稿信草稿已就绪，待目标期刊/作者信息确认）
