...
- **主创新**：证据表征受控消融（A 原始日志 / B 结构化证据 / C 反例语义化 / D FVDebug 式因果图），干净口径（无泄漏）下 A 64.7% > D 56.9% > B 55.9% > C 43.2%（95/102），效应被模型、数据卫生与反例时长共同调节；
- **差异化三支柱**：① 开源全链路可复现（iverilog/verilator/yosys/sby，无商业依赖）；② 四设置消融方法论（A/B/C/D 干净 408 次）+ 深时序子集（60/60 修复率 100%，排序反转）；③ 跨周期专项数据集（34 样本）+ 验证充分性闭环（mutation/非空洞/T2 审计/数据卫生）；
- **目标**：2026-10 论文初稿 + BugBench-PS 数据集（34 个 L3 样本，已定案）+ 开源工具链。
...
输入层(3类失败证据) → ①EvidenceEngine(结构化) → ②CexSemantizer(语义化★) → ③LocalRepairer(切片patch) → ④Verifier(三通过+充分性)
                          └──────────── 失败回灌（≤N 轮）────────────┘
贯穿层：DeepSeek v4-flash API ｜ Controller（实验变量，T4 MCP 工具层）
...
| BugBench-PS | 34 个 L3 样本数据集（s04–s37，6 模块 × 7 错误类型，golden 双对照） |
...
| --- | --- | --- |
| A | 原始日志/反例原文 | 基线 |
| B | 结构化 JSON | 消融（干净口径 55.9%，非最优） |
| C | 反例语义化（周期事件+轨迹+故障锥+NL） | 消融 |
| D | FVDebug 式因果图摘要 | 对照（成本最低 $0.37/102 次） |
| T1 | C + 多模态波形图通道 | 探索章节 |

## 快速开始

前置：WSL（工具链）+ Python 3.10+ + DeepSeek API Key（.env 配置）。
...
| W7–9 | M3+M4 | 主实验 A/B/C/D 408 次完成；Verifier 充分性（88.5%/81.8%）；跨模型 306 对 + L2 72 次 | Gate-2 通过 |
