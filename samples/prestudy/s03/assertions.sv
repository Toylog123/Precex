// PreCex - counter_alu L3 缺陷样本 s03 强断言（安全子集写法：always 块内 immediate assert + 寄存器打拍实现跨周期性质）
// 作者：Toylog | 版本：v0.1 | 功能概述：覆盖计数器仅使能自增/未使能保持/复位释放归 0/满值回绕/ALU 输出打拍正确性/运算选择不越界 6 条性质
// 来源：本文件为 rtl/counter_alu/assertions.sv 的黄金断言拷贝（未改动），保证与 rtl 断言矩阵一致
// 说明：本文件为独立模块，由 tb/formal_top 显式实例化
// 击穿点：A6（计数器满值回绕）命中 buggy 的"满值回 1"缺陷——cnt_d==全1 且使能时 cnt 必须回绕为 0
