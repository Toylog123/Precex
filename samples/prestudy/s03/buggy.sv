// PreCex - counter_alu L3 缺陷样本 s03（buggy 版，含内联强断言）
// 作者：Toylog | 版本：v0.2 | 功能概述：注入"边界回绕错误"缺陷——计数器满值（全 1）时回绕到 1 而非 0，
//        计数序列 254->255->1 错失 0，破坏模 2^DATA_W 自然回绕；ALU 部分与黄金一致；
//        断言内联于模块内（formal 友好写法：always @(posedge clk) 单边沿 + initial 初值，参照 smoke/counter.sv 已验证模式）
// 来源：基于 rtl/counter_alu/counter_alu.sv 注入单点缺陷；内联断言与 rtl/counter_alu/assertions.sv 性质一致（A1-A6）
// 击穿点：A6（计数器满值回绕：cnt_d==全1 且使能时 cnt 必须回绕为 0）——buggy 回绕到 1
...
module counter_alu #(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // ===== BUG (inject) =====
            // 缺陷：满值回绕逻辑改为"回绕到 1"——cnt 达全 1 时下一拍回 1 而非 0。
            // 正确行为（黄金）：模 2^DATA_W 自然回绕，即 cnt <= cnt + 1'b1;（255 + 1 = 0）。
            // 后果：计数序列 254->255->1 错失 0，破坏 8-bit 计数循环（边界回绕错误，跨周期暴露）。
            if (cnt == {DATA_W{1'b1}}) begin
                cnt <= {{(DATA_W-1){1'b0}}, 1'b1};   // BUG: 满值应回绕为 0
            end else begin
                cnt <= cnt + 1'b1;
            end
            // ===== /BUG =====
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            // A5 运算选择不越界
            assert (op < OP_NUM);
            // A6 计数器满值回绕：上周期 cnt 为全 1 且使能 → 本周期 cnt 必须回绕为 0
            // —— 击穿点：buggy 满值回绕到 1，违反 cnt == {DATA_W{1'b0}}
            if (cnt_en_d && (cnt_d == {DATA_W{1'b1}})) begin
                assert (cnt == {DATA_W{1'b0}});
            end
        end
    end
