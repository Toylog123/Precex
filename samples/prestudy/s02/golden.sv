// PreCex - fsm_ctrl L3 缺陷样本 s02（golden 版，含内联强断言）
// 作者：Toylog | 版本：v0.2 | 功能概述：3 状态序列控制器黄金基线（S1/S2/S3 分级停留 + 异常跳转 + 全局超时保护，done/timeout_irq 单拍脉冲）；
//        断言内联于模块内（formal 友好写法：always @(posedge clk) 单边沿 + initial 初值，参照 smoke/counter.sv 已验证模式）
// 来源：设计逻辑为 rtl/fsm_ctrl/fsm_ctrl.sv 的黄金基线拷贝（参数与 rtl 默认一致）；
//        内联断言与 rtl/fsm_ctrl/assertions.sv 性质一致（A1-A6）
