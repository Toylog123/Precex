// PreCex - fifo_sync L2 缺陷样本 l2_fifo_02（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『FIFO 满空』类缺陷——同拍读写 count 守恒破坏：删 can_wr&&can_rd 的保持分支（同拍读写 count 仍 +1，击穿 A4）
// 来源：rtl/fifo_sync/fifo_sync.sv 单点注入（行 54）| 击穿断言：fifo_sync A4（count 增量守恒）
