// PreCex - fifo_sync L2 缺陷样本 l2_fifo_06（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『边界回绕』类缺陷——写指针越步：tail+1 改 tail+2（跳过一个单元，击穿 A6 推进）
// 来源：rtl/fifo_sync/fifo_sync.sv 单点注入（行 47）| 击穿断言：fifo_sync A6（写指针推进）
