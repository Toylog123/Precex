// PreCex - fifo_sync L2 缺陷样本 l2_fifo_03（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『位宽截断』类缺陷——count 位宽收窄 1 位（count 存满值时截断回绕，击穿 A1/A4 满值与守恒）
// 来源：rtl/fifo_sync/fifo_sync.sv 单点注入（行 27）| 击穿断言：fifo_sync A1/A4
...
module fifo_sync #(
    ...
    reg [CNT_W-2:0] count;             // 已存元素数（直接计数，杜绝指针回绕错）
