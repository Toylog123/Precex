// PreCex - uart_rx L2 缺陷样本 l2_uartrx_04（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『状态跳转』类缺陷——uart_rx 数据位结束跳过 STOP 直接回 IDLE（帧无完成脉冲，击穿 A4）
// 来源：rtl/uart_rx/uart_rx.sv 单点注入（行 79）| 击穿断言：uart_rx A4（DATA→STOP 跳转）
