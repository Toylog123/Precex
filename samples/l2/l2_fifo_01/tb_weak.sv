...
module tb_fifo_sync;
    ...
    initial begin
        ...
        if (!half_full)  $fatal(1, "FAIL: half_full not asserted when count>=4");
        ...
        @(negedge clk);
        wr_en = 1'b0;
        if (uut.count !== 8) $fatal(1, "FAIL: write accepted while full");
        ...
        @(negedge clk);
        rd_en = 1'b0;
        if (uut.count !== 0) $fatal(1, "FAIL: read accepted while empty");
        ...
        if (uut.count !== 1) $fatal(1, "FAIL: count changed on concurrent wr+rd");
        if (dout !== 8'h11)  $fatal(1, "FAIL: concurrent read should return old data 0x11, got %0h", dout);
        ...
        @(negedge clk);
        rd_en = 1'b0;
        if (dout !== 8'h5A) $fatal(1, "FAIL: data order corrupted (expect 0x5A, got %0h)", dout);
        @(negedge clk);
        rd_en = 1'b1;
        @(negedge clk);
        rd_en = 1'b0;
        if (dout !== 8'h11) $fatal(1, "FAIL: data order corrupted (expect 0x11, got %0h)", dout);
