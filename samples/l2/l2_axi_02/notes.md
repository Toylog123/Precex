# l2_axi_02 - axi_lite_slave L2 缺陷样本构造说明
...
- 击穿断言：axi A6（读数据译码正确性）
- L2 双检校验：① iverilog 编译 0 error ✓；② 弱 tb 仿真 FAIL（弱 tb 可抓）✓；③ sby (smtbmc+z3) BMC 抓到反例 ✓（伪 L3 判据成立）
- 复现命令：`python3 scripts/bug_injector.py --module axi_lite_slave --error-type boundary_wrap --sample-id l2_axi_02 --variant 9`
