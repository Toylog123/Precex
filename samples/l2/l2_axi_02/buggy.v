// PreCex - axi_lite_slave L2 缺陷样本 l2_axi_02（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『边界回绕』类缺陷——axi 读地址译码偏移：ARADDR[ADDR_W-1:2] 改 [ADDR_W-2:1]（读数据错位，击穿 A6）
// 来源：rtl/axi_lite_slave/axi_lite_slave.sv 单点注入（行 144）| 击穿断言：axi A6（读数据译码正确性）
