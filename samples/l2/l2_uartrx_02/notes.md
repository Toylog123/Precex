# l2_uartrx_02 - uart_rx L2 缺陷样本构造说明
...
- 击穿断言：uart_rx A4（状态机跳转合法性）
- L2 双检校验：① iverilog 编译 0 error ✓；② 弱 tb 仿真 FAIL（弱 tb 可抓）✓；③ sby (smtbmc+z3) BMC 抓到反例 ✓（伪 L3 判据成立）
- 复现命令：`python3 scripts/bug_injector.py --module uart_rx --error-type state_trans --sample-id l2_uartrx_02 --variant 5 --param CLK_FREQ=400,BAUD=100`
