// PreCex - axi_lite_slave L2 缺陷样本 l2_axi_04（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『握手』类缺陷——写响应不释放：BVALID 释放分支删除（写响应持续有效，重复握手）
// 来源：rtl/axi_lite_slave/axi_lite_slave.sv 单点注入（行 116）| 击穿断言：axi 写通道握手断言（BVALID 不释放）
