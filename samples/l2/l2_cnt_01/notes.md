# l2_cnt_01 - counter_alu L2 缺陷样本构造说明
...
- 击穿断言：counter_alu A4（ALU 输出正确性）
- L2 双检校验：① iverilog 编译 0 error ✓；② 弱 tb 仿真 FAIL（弱 tb 可抓）✓；③ sby (smtbmc+z3) BMC 抓到反例 ✓（伪 L3 判据成立）
- 复现命令：`python3 scripts/bug_injector.py --module counter_alu --error-type state_trans --sample-id l2_cnt_01 --variant 8`
