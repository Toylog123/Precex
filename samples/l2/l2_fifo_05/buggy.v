// PreCex - fifo_sync L2 缺陷样本 l2_fifo_05（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『边界回绕』类缺陷——写指针不回绕推进：tail <= tail+1 改 tail 保持（同址覆写，数据顺序错乱）
// 来源：rtl/fifo_sync/fifo_sync.sv 单点注入（行 48）| 击穿断言：fifo_sync A4/A3 类指针性质
