// PreCex - fsm_ctrl L3 缺陷样本 s56（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『状态跳转』类缺陷——删除 S2 停留计数分支（hold_cnt==S2_HOLD 判断改无条件跳 S3，停留不足，修复需重建等待逻辑 insert_wait）
// 来源：rtl/fsm_ctrl/fsm_ctrl.sv 单点注入（行 1）| 击穿断言：fsm_ctrl 强断言 A8（S2 停留不足 S2_HOLD 拍不得离开）
...
// 端口方向/宽度体内声明（Verilog-2001 非 ANSI 风格）
    ...
    // [structural] A8 强断言：S2 停留不足 S2_HOLD 拍不得离开
    always @(posedge clk) begin
        if (rst_n && (state_d == S2) && (hold_cnt_d < S2_HOLD) && (state != S_IDLE)) begin
            assert (state == S2);
        end
    end
