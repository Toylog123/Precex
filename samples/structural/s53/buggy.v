// PreCex - uart_rx L3 缺陷样本 s53（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『状态跳转』类缺陷——起始位中点确认分支删除（跳过毛刺确认，修复需重建确认态 split_state）
// 来源：rtl/uart_rx/uart_rx.sv 单点注入（行 1）| 击穿断言：uart_rx A2（起始位中点确认后才进数据接收）
            ...
            case (state)
                ...
                S_START: begin
                    if (baud_cnt == (HALF - 1)) begin
                        // [structural] 毛刺检测分支被删除，中点无条件进 DATA
                        baud_cnt <= {DIV_W{1'b0}};
                        bit_cnt  <= 4'd0;
                        state    <= S_DATA;
