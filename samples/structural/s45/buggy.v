// PreCex - uart_tx L3 缺陷样本 s45（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『状态跳转』类缺陷——数据位结束跳转目标 S_STOP 改 S_IDLE（帧缺停止位，修复需重建 STOP 状态 split_state）
// 来源：rtl/uart_tx/uart_tx.sv 单点注入（行 1）| 击穿断言：uart_tx A4（DATA→STOP 收尾）
