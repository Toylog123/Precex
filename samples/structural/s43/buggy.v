// PreCex - fsm_ctrl L3 缺陷样本 s43（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『状态跳转』类缺陷——S1 停留满后跳转目标 S2 改 S3（跳过 S2 停留，修复需重建 S2 中间态/停留逻辑 split_state）
// 来源：rtl/fsm_ctrl/fsm_ctrl.sv 单点注入（行 1）| 击穿断言：fsm_ctrl A1（状态跳转合法性：S1->S3 非法）
