// PreCex - fifo_sync L3 缺陷样本 s46（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『FIFO 满空』类缺陷——删除同拍读写 count 守恒分支（同时读写时 count 仍 +1，修复需重建守恒保护 guard_boundary）
// 来源：rtl/fifo_sync/fifo_sync.sv 单点注入（行 1）| 击穿断言：fifo_sync A4（count 增量守恒）
