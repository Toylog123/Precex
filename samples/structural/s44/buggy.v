// PreCex - fsm_ctrl L3 缺陷样本 s44（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『边界回绕』类缺陷——删除全部超时保护分支（step_cnt>=TIMEOUT 保护移除，修复需重插 guard_boundary）
// 来源：rtl/fsm_ctrl/fsm_ctrl.sv 单点注入（行 1）| 击穿断言：fsm_ctrl 强断言 A8（step_cnt 超阈值后必须回 IDLE）
