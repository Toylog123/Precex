// PreCex - uart_rx L3 缺陷样本 s36（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『状态跳转』类缺陷——uart_rx 起始位确认后跳过数据位：START 中点确认后直接进 STOP（击穿 A4）
// 来源：rtl/uart_rx/uart_rx.sv 单点注入（行 67）| 击穿断言：uart_rx A4（状态机跳转合法性）
