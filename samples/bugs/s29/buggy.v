// PreCex - fifo_sync L3 缺陷样本 s32（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『FIFO 满空』类缺陷——写入动作整体删除：删 mem[tail]<=din 与 tail<=tail+1（写请求被吞但 count 仍 +1，击穿 A6）
// 来源：rtl/fifo_sync/fifo_sync.sv 单点注入（行 44）| 击穿断言：fifo_sync A6（写指针推进）
