// PreCex - uart_rx L3 缺陷样本 s37（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『边界回绕』类缺陷——uart_rx 起始位中点偏移：baud_cnt == HALF-1 改 == HALF（中点确认迟到一拍，击穿 A1/A2）
// 来源：rtl/uart_rx/uart_rx.sv 单点注入（行 60）| 击穿断言：uart_rx A1/A2（起始位中点确认）
...
module uart_rx #(
    parameter CLK_FREQ = 1600,   // 系统时钟频率 Hz
...
// 端口方向/宽度体内声明（Verilog-2001 非 ANSI 风格）
    ...
    always @(posedge clk) assume (!(state == S_START) || !rxd);
