// PreCex - uart_tx L3 缺陷样本 s41（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『状态跳转』类缺陷——uart_tx 数据位结束跳过 STOP 直接回 IDLE（帧缺停止位，击穿 A4）
// 来源：rtl/uart_tx/uart_tx.sv 单点注入（行 78）| 击穿断言：uart_tx A4（DATA→STOP 收尾）
...
module uart_tx #(
    parameter CLK_FREQ = 800,   // 系统时钟频率 Hz
