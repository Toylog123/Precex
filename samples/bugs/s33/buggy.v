// PreCex - axi_lite_slave L3 缺陷样本 s36（buggy 版）
// 作者：Toylog | 版本：v0.1 | 功能概述：注入『状态跳转』类缺陷——axi 写响应前置放宽：BVALID 仅需 aw_done（无写数据也响应，击穿 A3）
// 来源：rtl/axi_lite_slave/axi_lite_slave.sv 单点注入（行 114）| 击穿断言：axi A3（BVALID 前置 AW/W 完成）
...
module axi_lite_slave #(
    ...
    always @(posedge ACLK or negedge ARESETN) begin
        if (!ARESETN) begin
            S_AXI_BVALID <= 1'b0;
            S_AXI_BRESP  <= 2'b00;
        end else if (aw_done && !S_AXI_BVALID) begin
            S_AXI_BVALID <= 1'b1;
